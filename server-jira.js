...
var jirabase = 'http://find1.dyndns.org/jira';
var user = 'drende';
...
app.get('/ws/targets/:target/reports', function (req, res) {
    ...
    xhr.open("get", jirabase + '/rest/api/2/search?jql=target~"' + target + '"&fields=id,summary,issuetype,created,updated,attachment', true);
...
app.get('/ws/reports/:id', function (req, res) {
    ...
    xhr.open("get", jirabase + '/rest/api/2/issue/' + id, true);
    xhr.setRequestHeader('Authorization', make_basic_auth(user, password));
    xhr.send();
});
...
//    console.log("post /ws/reports " + req.body);
    ...
    xhr.open("post", jirabase + '/rest/api/2/issue', true);
    xhr.setRequestHeader('Authorization', make_basic_auth(user, password));
...
app.put('/ws/reports/:id', function (req, res) {
    ...
    xhr.open("put", jirabase + '/rest/api/2/issue/' + id, true);
    xhr.setRequestHeader('Authorization', make_basic_auth(user, password));
    xhr.setRequestHeader('Content-Type', 'application/json');
    xhr.send(JSON.stringify(issue));
});
...
function upload_attachments_to_jira(report) {
    for (i in report.attachments) {
        ...
        if (attachment.inJira == undefined) {
            ...
            var params = parseUrl(jirabase + '/rest/api/2/issue/' + report.id + '/attachments');
...
app.get('/ws/attachments/:id/raw', function (req, res) {
    var id = req.params.id;
    var url = jirabase + '/secure/attachment/' + id + '/';
